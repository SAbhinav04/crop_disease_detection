...
const T = {
  en: {
    ...
    stat2: 'Crop diseases',
    stat3: 'Multilingual support',
    tag1: 'Instant diagnosis',
    tag2: 'AI-powered',
    tag3: 'Kannada support',
...
export default function Login({ onLogin, initialLanguage = 'en', initialPhone = DEFAULT_BYPASS_PHONE }) {
  ...
  const completeLogin = () => {
    if (onLogin) onLogin({ phone, lang, name: canBypassLogin ? 'demo' : '' });
  };
      ...
      <main style={{ flex: 1, position: 'relative', zIndex: 10, display: 'flex', alignItems: 'center', padding: '2rem 2.5rem', gap: '3rem', maxWidth: 1100, margin: '0 auto', width: '100%' }}>
        ...
        <div style={{ width: 380, flexShrink: 0, background: 'rgba(240,243,237,0.92)', backdropFilter: 'blur(20px)', borderRadius: 20, border: '0.5px solid rgba(255,255,255,0.4)', padding: '2rem', boxShadow: '0 24px 60px rgba(0,0,0,0.25)' }}>
          <div style={{ display: 'flex', background: 'rgba(0,0,0,0.06)', borderRadius: 10, padding: 3, marginBottom: '1.5rem' }}>
            {['en', 'kn'].map((l) => (
              <button key={l} onClick={() => { setLang(l); setError(''); }} style={{ flex: 1, padding: '7px 0', fontSize: 13, fontWeight: 500, border: 'none', cursor: 'pointer', borderRadius: 8, background: lang === l ? '#fff' : 'transparent', color: lang === l ? '#1a2e0f' : 'rgba(0,0,0,0.45)', boxShadow: lang === l ? '0 1px 4px rgba(0,0,0,0.1)' : 'none', transition: 'all 0.15s' }}>{l === 'en' ? 'English' : 'ಕನ್ನಡ'}</button>
            ))}
          </div>
          ...
          {canBypassLogin ? <div style={{ fontSize: 11, color: '#3B6D11', marginBottom: '0.75rem', fontWeight: 500 }}>Demo login enabled for {DEFAULT_BYPASS_PHONE}</div> : null}
          ...
          {error ? <div style={{ fontSize: 12, color: '#c0392b', background: 'rgba(192,57,43,0.08)', borderRadius: 8, padding: '8px 12px', marginBottom: '0.75rem' }}>{error}</div> : null}
